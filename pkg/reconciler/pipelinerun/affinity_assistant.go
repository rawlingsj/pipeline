...
func (c *Reconciler) createAffinityAssistants(ctx context.Context, wb []v1beta1.WorkspaceBinding, pr *v1beta1.PipelineRun, namespace string) error {
	...
	for _, w := range wb {
		if w.PersistentVolumeClaim != nil || w.VolumeClaimTemplate != nil {
			...
			_, err := c.KubeClientSet.AppsV1().StatefulSets(namespace).Get(context.TODO(), affinityAssistantName, metav1.GetOptions{})
			...
			case apierrors.IsNotFound(err):
				...
				_, err := c.KubeClientSet.AppsV1().StatefulSets(namespace).Create(context.TODO(), affinityAssistantStatefulSet, metav1.CreateOptions{})
				if err != nil {
...
func (c *Reconciler) cleanupAffinityAssistants(ctx context.Context, pr *v1beta1.PipelineRun) error {
	...
	for _, w := range pr.Spec.Workspaces {
		if w.PersistentVolumeClaim != nil || w.VolumeClaimTemplate != nil {
			...
			if err := c.KubeClientSet.AppsV1().StatefulSets(pr.Namespace).Delete(context.TODO(), affinityAssistantStsName, metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
