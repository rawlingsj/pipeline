...
func createPVC(ctx context.Context, pr *v1beta1.PipelineRun, c kubernetes.Interface) (*corev1.PersistentVolumeClaim, error) {
	if _, err := c.CoreV1().PersistentVolumeClaims(pr.Namespace).Get(context.TODO(), GetPVCName(pr), metav1.GetOptions{}); err != nil {
		if errors.IsNotFound(err) {
			...
			pvc, err := c.CoreV1().PersistentVolumeClaims(pr.Namespace).Create(context.TODO(), pvcSpec, metav1.CreateOptions{})
			if err != nil {
...
func deletePVC(pr *v1beta1.PipelineRun, c kubernetes.Interface) error {
	...
	} else if err := c.CoreV1().PersistentVolumeClaims(pr.Namespace).Delete(context.TODO(), GetPVCName(pr), metav1.DeleteOptions{}); err != nil {
